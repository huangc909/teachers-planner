...
const OtherSchoolYear = (props) => {
  ...
  const day = new Date(`August 1, ${startYear} 00:00:00`)
  const todaysYear = day.getFullYear()
  // console.log('todaysYear: ', todaysYear)
  const leapYear = (todaysYear % 100 === 0) ? (todaysYear % 400 === 0) : (todaysYear % 4 === 0)
  // console.log('leapYear: ', leapYear)
  const schoolYearMonthNumber = 1
  // console.log('schoolYearMonthNumber: ', schoolYearMonthNumber)
  const todaysMonthName = 'August'
  ...
  const todaysDayNumber = day.getDay()
  ...
  // Need to pass a value in for prevDateId since there won't be any for august 1
  let prevDateId = ''
  if (currDate === 1 && currMonthName === 'August') {
    prevDateId = 0
  } else if (currDate === 1) {
...
export default OtherSchoolYear
