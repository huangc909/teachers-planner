...
const PrevDay = props => {
  ...
  // Set up prev day as current day
  // Calculate prev day, if it is the end of prev month
  // if (currDate === 1) {
  //   if (currMonthIndex === 0) {
  //     currMonthIndex = 11
  //   } else {
  //     currMonthIndex--
  //   }
  //   currDate = prevMonth.length
  // } else {
  //   currDate--
  // }

  if (currMonthIndex === 0 && currDate === 1) currMonthIndex = 11
  ...
  useEffect(() => {
    axios({
      ...
      .then(res => setPrevDay(res.data.day))
      .then(() => msgAlert({
  ...
  const dailyTasks = prevDay.tasks.map(task => (
    <CheckMark
      {...props}
      key={task._id}
      task={task}
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <div style={{ margin: '10px' }}>
        {prevDay.length === 0 ? <div></div> : dailyTasks}
      </div>
      <div>
        <Link to={{
